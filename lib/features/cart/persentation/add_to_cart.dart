...
class _AddToCartState extends State<AddToCart> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 100), () {
      setState(() {
        ...
        print('check widget data menu, ${widget.dataMenu}');
        if (widget.dataMenu.containsKey('id_varian')) {
          setState(() {
            selectedVarian = {
              'id_varian': widget.dataMenu['id_varian'],
              'nama_varian': widget.dataMenu['nama_varian'],
              'harga_varian': widget.dataMenu['harga_varian'],
            };
          });
          if (widget.dataMenu.containsKey('pref')) {
            setState(() {
              selectedPref = widget.dataMenu['pref'];
            });
          }
          if (widget.dataMenu.containsKey('addon')) {
            setState(() {
              selectedAddon = widget.dataMenu['addon'];
              int idx = 0;
              selectedAddon.forEach((element) {
                int idx2 = 0;
                addonDisplay.forEach((element2) {
                  if (element['id_menu'] == element2['id_menu']) {
                    qtyAddonController[idx2].text =
                        selectedAddon[idx]['qty'].toString();
                  }
                  idx2++;
                });
                idx++;
              });
              // qtyAddonController.forEach((element) {
              //   // if (selectedAddon[idx].containsKey('id_menu')) {
              //   // }
              //   idx++;
              // });
            });
          }
          if (widget.dataMenu.containsKey('notes')) {
            setState(() {
              notes = widget.dataMenu['notes'];
              notesController.text = widget.dataMenu['notes'];
            });
          }

          if (widget.dataMenu.containsKey('qty')) {
            setState(() {
          ...
          if (widget.isNew == false) {
            CartItem cartData = cart.getItemByIndex(widget.dataMenu['index']);
            cartId = cartData.id;
          }
          // selectedVarian.id_varian = widget.dataMenu['id_varian'];
          // selectedVarian['id_varian'] = widget.dataMenu['id_varian'];
        }
      });
    });
  ...
  void checkSelectedAddon(int index, int qty) {
    ...
    if (qty > 0) {
      if (checkSameField(selectedAddon, 'id_menu', tmp['id_menu']) == null) {
        tmp['qty'] = qty;
  ...
  void addToCart(
    ...
    String id = item['id_menu'];

    if (selectedVarian != null) {
      id += "-v${varian['id_varian']}";
    }
    ...
    if (selectedPref.isNotEmpty) {
      selectedPref.forEach((value) {
        int currentIndex = prefDisplay.indexWhere((elem) =>
            elem['id'] == value['id'] && elem['type'] == value['type']);
        if (currentIndex != null) {
          id += "-p${currentIndex}";
        }
        // print('check value, $currentIndex');
        // print('check key, $key');
      });
    }

    print('check id, $id');
    print('check select pref, $selectedPref');

    CartItem newItem = CartItem(
      id: id,
      name: item['id_menu'],
      itemName: item['nama_menu'],
      variantId: varian['id_varian'],
      variant: varian['nama_varian'],
      variantPrice: varian['harga_varian'].toInt(),
      notes: note,
      preference: {},
      pref: selectedPref,
  ...
  void onEditCart(
    ...
    //   id
    // }
    cart.removeItemm(cartId);

    CartItem editItem = CartItem(
      id: id,
      name: item['id_menu'],
      itemName: item['nama_menu'],
      variantId: varian['id_varian'],
    ...
    setState(() {
      cart.addItem(editItem, mode: CartMode.add);
    });
